...
class PPOAgent(object):
    ...
    def step(self, state):
        obs = torch.Tensor(state['obs']).unsqueeze(0)
        legal_actions = list(state['legal_actions'])
        policy_values = self.actor_net(obs).squeeze(0)
        legal_actions_values = []

        for i in legal_actions:
            legal_actions_values.append(policy_values[i])

        best_actions_index = np.argmax(legal_actions_values)

        return legal_actions[best_actions_index]
    ...
    def eval_step(self, state):
        ...
        best_action = legal_actions[np.argmax(legal_actions_values)]

        info = {'values': {state['raw_legal_actions'][i]: float(policy_values[list(state['legal_actions'].keys())[i]])
    ...
    def train(self, trajectory):
        ...
        obs_array = [state['obs'] for state in states]
        trajectory_obs = torch.Tensor(obs_array).to(self.device)
        ...
        self.writer.add_scalar('mean_100episodes_reward', mean_episodes_reward, self.total_steps)
    ...
    def unpack(self, trajectory):
        ...
        for i in range(len(trajectory)):
            del trajectory[i][-2]
            del trajectory[i][0]['raw_obs']
            del trajectory[i][0]['raw_legal_actions']
